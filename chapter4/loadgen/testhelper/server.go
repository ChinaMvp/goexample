...
// reqHandler 把参数sresp代表的请求转换为数据，并发送给连接。
func reqHandler(conn net.Conn) {
	var errMsg string
	var sresp ServerResp
	req, err := read(conn, DELIM) //从连接中读取内容
	if err != nil {
		errMsg = fmt.Sprintf("Server: Req Read Error: %s", err)
	} else {
		var sreq ServerReq
		err := json.Unmarshal(req, &sreq) //解码json字符串
		if err != nil {
			errMsg = fmt.Sprintf("Server: Req Unmarshal Error: %s", err)
		} else {
			sresp.ID = sreq.ID
			sresp.Result = op(sreq.Operands, sreq.Operator) //根据操作数和操作符进行计算
			sresp.Formula = genFormula(sreq.Operands, sreq.Operator, sresp.Result, true) //表示计算过程的字符串
		}
	}
	if errMsg != "" {
		sresp.Err = errors.New(errMsg)
	}
	bytesJson, err := json.Marshal(sresp) //进行json编码
	if err != nil {
		logger.Errorf("Server: Resp Marshal Error: %s", err)
	}
	_, err = write(conn, bytesJson, DELIM)
	if err != nil {
