...
func NewGenerator(pset ParamSet) (lib.Generator, error) {
	...
	gen := &myGenerator{
		caller:     pset.Caller,  // 调用器。
		timeoutNS:  pset.TimeoutNS,  // 响应超时时间，单位：纳秒。
		lps:        pset.LPS,  // 每秒载荷量。
		durationNS: pset.DurationNS,  // 负载持续时间，单位：纳秒。
		status:     lib.STATUS_ORIGINAL, //初始状态
		resultCh:   pset.ResultCh, // 调用结果通道。
	}
	if err := gen.init(); err != nil { // 初始化载荷发生器。
		return nil, err
	}
	return gen, nil
}
...
func (gen *myGenerator) init() error {
	...
	gen.concurrency = uint32(total64) // 载荷的并发量
	tickets, err := lib.NewGoTickets(gen.concurrency)
	if err != nil {
		return err
	}
	gen.tickets = tickets // Goroutine票池。
