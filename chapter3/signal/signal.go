...
func handleSignal() {
	...
	go func() {
		for sig := range sigRecv1 {
			fmt.Printf("Process (pid=%d) received a signal from sigRecv1: %s\n", os.Getpid(), sig)
		}
	...
	go func() {
		for sig := range sigRecv2 {
			fmt.Printf("Process (pid=%d) received a signal from sigRecv2: %s\n", os.Getpid(), sig)
		}
...
func runCmds(cmds []*exec.Cmd) ([]string, error) {
	...
	for _, cmd := range cmds {
		...
		if !first { //if not first, get input from last output
